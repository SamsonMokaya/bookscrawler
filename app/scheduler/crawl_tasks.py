...
async def save_book_to_db(book_data: Dict) -> Dict[str, any]:
    """
    Save or update a book in the database with change detection
    ...
    try:
        ...
        existing_book = await Book.find_one(Book.source_url == book_data['source_url'])
        ...
        if existing_book:
            ...
            changes_saved = 0
            if changes:
                changes_saved = await save_changes_to_log(changes)
...
def crawl_all_books_task(self, start_page: int = 1, end_page: int = None):
    """
    Celery task to crawl all books from the website
